...
from meter import Meter,MetaMeter,MetaCasseteMeter,MetaSpectrumMeter,MetaMSpectrumWithMeter,TunerSpectrumWithMeter,\
    MetaNadDeckMeter,MetaPioReelMeter,MetaAkaiDeckMeter,MetaTeacDeckMeter,MetaPioDeckMeter,MetaPhonoMeter
...
class MeterFactory(object):
    ...
    def create_meter(self):
        ...
        if meter_config_section[METER_TYPE] == TYPE_METALINEAR:
            ...
            #return self.create_metacircular_meter(meter_name)
            return self.create_tunercirclesspectrum_meter(meter_name)
    ...
    def create_metacircular_meter(self, name):
        ...
        if config[CHANNELS] == 2:
            if name == 'pioreel':
                ...
                meter = MetaPhonoMeter(self.util, TYPE_CIRCULAR, config, self.data_source)
            else:
    ...
    def create_metacirclesspectrum_meter(self, name):
        ...
        return meter
    def create_tunercirclesspectrum_meter(self, name):
        """ Create circular method
        ...
        if config[CHANNELS] == 2:
            meter = TunerSpectrumWithMeter(self.util, TYPE_CIRCULAR, config, self.data_source)
            meter.channels = 2
        else:
            meter = Meter(self.util, TYPE_CIRCULAR, config, self.data_source)
